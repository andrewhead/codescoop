...
module.exports.MainController = class MainController
  ...
  constructor: (codeEditor, exampleEditor) ->
    ...
    for range in snippetRanges
      middleText += ((codeEditor.getTextInBufferRange range) + "\n")
    ...
    exampleEditor.setText startText + middleText + endText

    exampleEditor.selectAll()
    selection = codeEditor.getLastSelection()
    exampleEditor.autoIndentSelectedRows()
    selection.clear()
    ...
    for panel in atom.workspace.getRightPanels()
      panel.destroy() if panel.item instanceof FakeController
    atom.views.addViewProvider FakeController, (controller) =>
      (new ControllerView exampleEditor).getNode()
    ...
    atom.workspace.addRightPanel { item: controller }
